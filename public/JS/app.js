...
(()=>{
    function storeWater(e){
        ...
        if(waterIn){
            ...
            if(parseInt(addWater) >= parseInt(goal)){
                alert("목표를 달성하였습니다 !");
    ...
    window.addEventListener('submit', (e)=>{
        ...
        var goalIn = document.querySelector('.goalIn').innerHTML;

        e.preventDefault();

        if (e.target === recStoreForm){
            // record.html 하단에 있는 마신 물 양 저장 함수
            // 기록할 양이 입력됐을 경우에만 저장함수 작동
            if(goalIn){
                storeWater();
            } else {
                alert('목표량이 작성되지 않았습니다. 기록 전 페이지 상단에서 목표량을 작성해주세요');
            }
            // waveWater();
        }
    })
